...
public class MyHashTable {
	...
	public EmployeeInfo searchByEmployeeNumber(int employeeNum) {
		...
		for (int j = 0; j < buckets[targetBucket].size(); j++) {
			if (employeeNum == buckets[targetBucket].get(j).getEmpNum()) {
				return(FTE) (buckets[targetBucket].get(j)); // The employee number is for the employee at position j in the target bucket's ArrayList.
			}			
        }
                return(null);
        }
		...
		for (int j = 0; j < buckets[targetBucket].size(); j++) {
			if (employeeNum == buckets[targetBucket].get(j).getEmpNum()) {
				return(PTE) (buckets[targetBucket].get(j)); // The employee number is for the employee at position j in the target bucket's ArrayList.
			}			
        }
                return(null);
        }
